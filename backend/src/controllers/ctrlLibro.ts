...
export const crearLibro = async (
  ...
  try {
    ...
    stream.on("finish", async () => {
      ...
      const imageUrl = `https://firebasestorage.googleapis.com/v0/b/adpowereat.appspot.com/o/libros%2F${encodeURIComponent(
        file.originalname
      )}?alt=media`;
      ...
      res
        .status(201)
        .json({ msg: "Libro creado correctamente", libro: nuevoLibro });
    });
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on("finish", () => {
      const imageUrl = `https://firebasestorage.googleapis.com/v0/b/${
        bucket.name
      }/o/libros%2F${encodeURIComponent(file.originalname)}?alt=media`;
      resolve(imageUrl);
    });
